...
for randSequence in args.seq:
  ...
  xSceneData=np.load("/system/user/mayr-data/BGNNRuns/trajectories/"+rp["problem"]+"/"+rp["experiment"]+"/"+saveModelName+"/preds_"+str(randSequence)+"_"+str(startTime)+".npy")
  ...
  for ind in range(len(xParticleData)):
    if (ind*spgap)%tsModulo==0:
      ...
      writer=vtk.vtkXMLPolyDataWriter()
      writer.SetFileName(os.path.join(rolloutFilePrefix, "wall_"+str((ind*spgap*rp["stepSize"])+1)+".vtp"))
