...
rolloutScript=os.path.join(processGitDir, "ablations", "rolloutStaticAb2.py")

exec(open(utilScript).read(), globals())

rp=collections.OrderedDict()
rp["version"]="processHopper1Ab2"
rp["problem"]="hopper"
rp["experiment"]="runs1"
rp["stepSize"]=1
...
if not restart:
  ...
  saveName="d"+str(contNr)+"_"+str(int(time.time()*1000.0))+"_"+datetime.date.today().strftime("%d-%m-%Y")+"_hopper1Ab2"
  if args.addInfo!="":
...
assert(rp["version"]=="processHopper1Ab2")
