...
parser.add_argument('--execRollout', help='', type=eval, choices=[True, False], default='True')
...
rolloutScript=os.path.join(processGitDir, "ablations", "rolloutStaticAb3.py")

exec(open(utilScript).read(), globals())

rp=collections.OrderedDict()
rp["version"]="processHopper2Ab3"
rp["problem"]="hopper"
rp["experiment"]="runs2"
rp["stepSize"]=1
...
if not restart:
  ...
  saveName="d"+str(contNr)+"_"+str(int(time.time()*1000.0))+"_"+datetime.date.today().strftime("%d-%m-%Y")+"_hopper2Ab3"
  if args.addInfo!="":
...
assert(rp["version"]=="processHopper2Ab3")
...
sampledPoints, nvecs=run_graph(tcoord, useDiam, nvec)

sampledPoints=sampledPoints.numpy()
