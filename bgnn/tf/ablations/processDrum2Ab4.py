...
parser.add_argument("-saveName", help="", type=str, default="abl3")
parser.add_argument("-addInfo", help="", type=str, default="Ab3")
parser.add_argument("-problem", help="", type=str, default="drum")
...
rolloutScript=os.path.join(processGitDir, "ablations", "rolloutDrumAb3.py")

exec(open(utilScript).read(), globals())

rp=collections.OrderedDict()
rp["version"]="processDrum2Ab3"
rp["problem"]="drum"
rp["experiment"]="runs2"
rp["stepSize"]=1
rp["noiseConstraint"]="V2"
rp["noisyGraph"]=True
rp["noisyTarget"]=True
rp["ZNoise"]=0
...
rp["multAngle"]=0.0
rp["gravBias"]=False
rp["model"]="modelDrum2.py"
rp["confFile"]="confDrum2.py"
rp["options"]="ablations/featureOptions1Ab3.py"
...
if not restart:
  ...
  saveName="d"+str(contNr)+"_"+str(int(time.time()*1000.0))+"_"+datetime.date.today().strftime("%d-%m-%Y")+"_drum2Ab3"
  if args.addInfo!="":
...
assert(rp["version"]=="processDrum2Ab3")
#rp["problem"]=args.problem
#rp["experiment"]=args.experiment
#rp["version"]="processDrum2"
#rp["model"]="modelDrum2.py"
#rp["confFile"]="confDrum2.py"
...
@tf.function
def run_graph(tcoordf, useDiam, nvec):
        edge01=tcoordf[:,1,:]-tcoordf[:,0,:]
        edge11=tcoordf[:,2,:]-tcoordf[:,0,:]
        edge01=edge01/tf.sqrt(tf.reduce_sum(edge01**2,1, keepdims=True))
        edge11=edge11/tf.sqrt(tf.reduce_sum(edge11**2,1, keepdims=True))
        ang1=tf.math.acos(tf.reduce_sum(edge01*edge11, 1))
        
        edge02=tcoordf[:,0,:]-tcoordf[:,1,:]
        edge12=tcoordf[:,2,:]-tcoordf[:,1,:]
        edge02=edge02/tf.sqrt(tf.reduce_sum(edge02**2,1, keepdims=True))
        edge12=edge12/tf.sqrt(tf.reduce_sum(edge12**2,1, keepdims=True))
        ang2=tf.math.acos(tf.reduce_sum(edge02*edge12, 1))
        
        edge03=tcoordf[:,1,:]-tcoordf[:,2,:]
        edge13=tcoordf[:,0,:]-tcoordf[:,2,:]
        edge03=edge03/tf.sqrt(tf.reduce_sum(edge03**2,1, keepdims=True))
        edge13=edge13/tf.sqrt(tf.reduce_sum(edge13**2,1, keepdims=True))
        ang3=tf.math.acos(tf.reduce_sum(edge03*edge13, 1))
        
        angsort=tf.argsort(tf.stack([ang1, ang2, ang3],0),0)
        tcoord=tf.stack([tf.gather(tcoordf, indices=angsort[0], batch_dims=1), tf.gather(tcoordf, indices=angsort[1], batch_dims=1), tf.gather(tcoordf, indices=angsort[2], batch_dims=1)],1)
        
        edge0=tcoord[:,1,:]-tcoord[:,0,:]
        edge1=tcoord[:,2,:]-tcoord[:,0,:]
        normEdge0=tf.sqrt(tf.reduce_sum(edge0**2,1, keepdims=True))
        normEdge1=tf.sqrt(tf.reduce_sum(edge1**2,1, keepdims=True))
        uEdge0=edge0/normEdge0
        uEdge1=edge1/normEdge1
        halfEdge=uEdge0+uEdge1
        normHalfEdge=tf.sqrt(tf.reduce_sum(halfEdge**2,1, keepdims=True))
        uHalfEdge=halfEdge/normHalfEdge
        lenE0=tf.reduce_sum(uHalfEdge*edge0,1, keepdims=True)
        lenE1=tf.reduce_sum(uHalfEdge*edge1,1, keepdims=True)
        
        vt0=tf.where(lenE0>=lenE1, edge0, edge1)
        vt1=tf.where(lenE0<lenE1, edge0, edge1)
        vt2=vt1-vt0
        
        normVT0=tf.sqrt(tf.reduce_sum(vt0**2,1, keepdims=True))
        normVT1=tf.sqrt(tf.reduce_sum(vt1**2,1, keepdims=True))
        normVT2=tf.sqrt(tf.reduce_sum(vt2**2,1, keepdims=True))
        
        uVT0=vt0/normVT0
        uVT1=vt1/normVT1
        uVT2=vt2/normVT2
        
        lenVT0=tf.reduce_sum(uHalfEdge*vt0,1, keepdims=True)
        lenVT1=tf.reduce_sum(uHalfEdge*vt1,1, keepdims=True)
        
        dirVec1=(lenVT1*uHalfEdge)-vt1
        normDirVec1=tf.sqrt(tf.reduce_sum(dirVec1**2,1, keepdims=True))
        uDirVec1=dirVec1/normDirVec1
        
        f1=vt0[:,1]*vt2[:,0]-vt0[:,0]*vt2[:,1]
        f2=-uHalfEdge[:,1]*vt0[:,0]+uHalfEdge[:,0]*vt0[:,1]
        f3=uHalfEdge[:,1]*vt2[:,0]-uHalfEdge[:,0]*vt2[:,1]
        lam=f1/f3
        mu=f2/f3
        intPoint1=tf.reshape(lam,(-1,1))*uHalfEdge
        #vt0+tf.reshape(mu,(-1,1))*vt2
        intPoint=intPoint1
        
        f1=vt0[:,1]*vt2[:,2]-vt0[:,2]*vt2[:,1]
        f2=-uHalfEdge[:,1]*vt0[:,2]+uHalfEdge[:,2]*vt0[:,1]
        f3=uHalfEdge[:,1]*vt2[:,2]-uHalfEdge[:,2]*vt2[:,1]
        lam=f1/f3
        mu=f2/f3
        intPoint2=tf.reshape(lam,(-1,1))*uHalfEdge
        #vt0+tf.reshape(mu,(-1,1))*vt2
        intPoint=tf.where(tf.math.is_nan(intPoint), intPoint2, intPoint)
        
        f1=vt0[:,0]*vt2[:,2]-vt0[:,2]*vt2[:,0]
        f2=-uHalfEdge[:,0]*vt0[:,2]+uHalfEdge[:,2]*vt0[:,0]
        f3=uHalfEdge[:,0]*vt2[:,2]-uHalfEdge[:,2]*vt2[:,0]
        lam=f1/f3
        mu=f2/f3
        intPoint3=tf.reshape(lam,(-1,1))*uHalfEdge
        #vt0+tf.reshape(mu,(-1,1))*vt2
        intPoint=tf.where(tf.math.is_nan(intPoint), intPoint3, intPoint)
        normIntPoint=tf.sqrt(tf.reduce_sum(intPoint**2,1, keepdims=True))
        
        #intPoint=intPoint+tcoord[:,0,:]
        
        descT12=tf.sqrt(normDirVec1**2+lenVT1**2)/lenVT1
        
        #Part 1&2
        #equal
        uVT0*descT12*lenVT1-dirVec1
        uVT1*descT12*lenVT1+dirVec1
        lenVT1*uHalfEdge
        
        uVT0*descT12*lenVT1 #segment 1 auf vt0: zwischen 0 und lenVT1 auf Winkelhalbierender
        uVT1*descT12*lenVT1 #segment 1 auf vt1: zwischen 0 und lenVT1 auf Winkelhalbierender
        
        
        
        dirVec0=-(normIntPoint/lenVT1)*dirVec1
        normDirVec0=tf.sqrt(tf.reduce_sum(dirVec0**2,1, keepdims=True))
        uVT0*descT12*normIntPoint+dirVec0 #segment2 auf vt0
        normIntPoint*uHalfEdge
        uVT0*descT12*lenVT1+uVT0*descT12*(normIntPoint-lenVT1)+dirVec0
        
        uVT0*descT12*lenVT1+uVT0*descT12*(normIntPoint-lenVT1) #segment2 auf vt0
        vt1-uVT2*((tf.sqrt(normDirVec1**2+(normIntPoint-lenVT1)**2)/(normIntPoint-lenVT1)))*(normIntPoint-lenVT1) #segment2 auf vt1
        
        descT45=tf.sqrt(normDirVec1**2+(normIntPoint-lenVT1)**2)/(normIntPoint-lenVT1)
        uVT0*descT12*(lenVT1+(normIntPoint-lenVT1)) #segment2 auf vt0: zwischen lenVT1 und normIntPoint auf Winkelhalbierender
        vt1-uVT2*descT45*(normIntPoint-lenVT1) #segment1 auf vt2: zwischen lenVT1 und normIntPoint auf Winkelhalbierender
        
        
        uVT0*descT12*(normIntPoint+(lenVT0-normIntPoint)) #segment3 auf vt0: zwischen normIntPoint und lenVT0 auf Winkelhalbierender
        intPoint-uVT2*descT45*(lenVT0-normIntPoint) #segment2 auf vt2: zwischen normIntPoint und lenVT0 auf Winkelhalbierender
        
        
        lenVT0Np=lenVT0.numpy()
        lenVT1Np=lenVT1.numpy()
        normIntPointNp=normIntPoint.numpy()
        
        start=[]
        end=[]
        nvecs=[]
        nrLines=tf.reshape(tf.cast(tf.math.ceil(lenVT0/useDiam), tf.int64),-1).numpy()
        subLine=(tf.reshape(lenVT0,-1)/tf.cast(nrLines, tf.float32)).numpy()
        i=0
        
        for i in range(0, nrLines.shape[0]):
          #lspace=tf.linspace(0.0, lenVT0Np[i], nrLines[i])
          lspace=tf.range(0.0, (nrLines[i]-0.5)*subLine[i], delta=subLine[i], dtype=tf.float32)+((lenVT0[i,0]-tf.cast(nrLines[i]-1, tf.float32)*subLine[i])/2.0)
          lspace1=lspace[tf.logical_and(lspace>=0.0,lspace<lenVT1Np[i])]
          lspace2=lspace[tf.logical_and(lspace>=lenVT1Np[i],lspace<normIntPointNp[i])]
          lspace3=lspace[tf.logical_and(lspace>=normIntPointNp[i],lspace<=lenVT0Np[i])]
          
          part11=tf.reshape(uVT0[i]*descT12[i], (-1,1))*tf.reshape(lspace1, (1,-1)) #segment 1 auf vt0: zwischen 0 und lenVT1 auf Winkelhalbierender
          part12=tf.reshape(uVT1[i]*descT12[i], (-1,1))*tf.reshape(lspace1, (1,-1)) #segment 1 auf vt1: zwischen 0 und lenVT1 auf Winkelhalbierender
          
          if np.isinf(descT45[i]):
            part1=tf.concat([part11], 1)
            part2=tf.concat([part12], 1)
          else:
            part21=tf.reshape(uVT0[i]*descT12[i], (-1,1))*tf.reshape((lenVT1[i]+(lspace2-lenVT1[i])), (1,-1)) #segment2 auf vt0: zwischen lenVT1 und normIntPoint auf Winkelhalbierender
            part22=tf.reshape(vt1[i], (3,-1))-tf.reshape(uVT2[i]*descT45[i], (-1,1))*tf.reshape((lspace2-lenVT1[i]), (1,-1)) #segment1 auf vt2: zwischen lenVT1 und normIntPoint auf Winkelhalbierender
            
            part31=tf.reshape(uVT0[i]*descT12[i], (-1,1))*tf.reshape((normIntPoint[i]+(lspace3-normIntPoint[i])), (1,-1)) #segment3 auf vt0: zwischen normIntPoint und lenVT0 auf Winkelhalbierender
            part32=tf.reshape(intPoint[i], (3,-1))-tf.reshape(uVT2[i]*descT45[i], (-1,1))*tf.reshape((lspace3-normIntPoint[i]), (1,-1)) #segment2 auf vt2: zwischen normIntPoint und lenVT0 auf Winkelhalbierender
            
            part1=tf.concat([part11, part21, part31], 1)
            part2=tf.concat([part12, part22, part32], 1)
          
          if np.any(np.isnan(part1)):
            raise Excpetion()
          elif np.any(np.isnan(part2)):
            raise Excpetion()
          
          start.append(part1+tf.reshape(tcoord[i,0,:], (-1, 1)))
          end.append(part2+tf.reshape(tcoord[i,0,:], (-1, 1)))
          nvecs.append(tf.repeat(tf.reshape(nvec[i], (1,3)), start[-1].shape[1], 0))
        
        start=tf.concat(start,1)
        end=tf.concat(end,1)
        nvecs=tf.concat(nvecs, 0)
        nvecsNew=[]
        
        lenLine=tf.sqrt(tf.reduce_sum((end-start)**2,0))
        nrPoints=tf.cast(tf.math.ceil(lenLine/useDiam), tf.int64).numpy()
        nrPoints[nrPoints<0.5]=1
        middle=(start+end)/2.0
        unitVec=(end-start)/lenLine
        subDist=(lenLine/tf.cast(nrPoints, tf.float32)).numpy()
        middle=tf.expand_dims(middle,0)
        unitVec=tf.expand_dims(unitVec,0)
        sampledPoints=[]
        for i in range(0, subDist.shape[-1]):
          if nrPoints[i]<1.5:
            sampledPoints.append(middle[:,:,i])
            nvecsNew.append(tf.repeat(tf.reshape(nvecs[i], (1,3)), sampledPoints[-1].shape[0], 0))
          else:
            symAdd=np.arange(0,(nrPoints[i]-0.5)*subDist[i], subDist[i])+((lenLine[i]-((nrPoints[i]-1)*subDist[i]))/2.0)-(lenLine[i]/2.0)
            symAdd=tf.cast(symAdd, tf.float32)
            symAdd=tf.expand_dims(symAdd,1)
            sampledPoints.append((middle[:,:,i]+symAdd*unitVec[:,:,i]))
            nvecsNew.append(tf.repeat(tf.reshape(nvecs[i], (1,3)), sampledPoints[-1].shape[0], 0))
        sampledPoints=tf.concat(sampledPoints,0)
        nvecsNew=tf.concat(nvecsNew, 0)
        return sampledPoints, nvecsNew

tf.config.run_functions_eagerly(True)

currentConstPositionVec=tf.identity(np.float32(xSceneData[0,:]))
...
drumDiameter=0.01*2.0*3
hopperDiameter=0.002*2.0*3

if args.problem=="drum":
  useDiam=drumDiameter
elif args.problem=="hopper":
  useDiam=hopperDiameter

sampledPointsOrig, nvecsOrig=run_graph(tcoord, useDiam, nvec)

sampledPointsOrig=sampledPointsOrig.numpy()
...
maxMBAbort=len(xParticleDataList[0:30])*(5000-(rp["nrPastVelocities"]+1))
...
for epochNr in range(startEpoch, min(maxEpochs, maxEpochsAbort)):
  if "trainNr" in dir() and trainNr!=min(len(trainList), maxMBAbort)-1:
    ...
    trainList=list(itertools.product(list(range(len(xParticleDataList[0:30]))), list(range((rp["nrPastVelocities"]+1)*rp["stepSize"], 5000))))
  ...
  for trainNr in range(startTrainNr, min(maxMB, maxMBAbort), rp["batchSize"]):
    ...
    for batchInd in range(0, min(rp["batchSize"], len(trainList)-trainNr)):
      ...
      data_output=1e-3
      rotation_period=4.0 #rpPar[randSequence]
      alpha=((2.0*np.pi)/float(rotation_period))*data_output*(timestep-1*rp["stepSize"])
      myMoveRot=np.array([[np.cos(alpha), 0.0, np.sin(alpha)], [0.0, 1.0, 0.0], [-np.sin(alpha), 0.0, np.cos(alpha)]])
      
      sampledPoints=np.float32(np.matmul(sampledPointsOrig, myMoveRot.T))
      nvecs=np.float32(np.matmul(nvecsOrig, myMoveRot.T))
      
      
      
      #useCutoff=tf.constant(np.array([[rp["neighborCutoff"]]]), dtype=tf.float32) 
      #points=currentPositionVecPerturbated
      #matTakeBoundary=tf.where(tf.sqrt(tf.reduce_sum((tf.reshape(points, (-1,1,3))-tf.reshape(sampledPoints, (1,-1,3)))**2, 2))<useCutoff)
      #matTakeBoundary=matTakeBoundary+tf.reshape(tf.constant(np.array([0,nrParticles])), (1,2))
      #srInd_1=matTakeBoundary[:,0].numpy()
      #srInd_2=matTakeBoundary[:,1].numpy()
      #print("wallParticleSenders2: "+str(len(tf.unique(matTakeBoundary[:,1])[0])))
      
      
      useCutoff=rp["neighborCutoff"]
      myTree=sklearn.neighbors.KDTree(currentPositionVecPerturbated.numpy())
      recvTree=myTree.query_radius(sampledPoints, r=useCutoff)
      sendTree=[np.repeat(i, len(recvTree[i])) for i in range(len(recvTree))]
      pwReceivers=np.concatenate(recvTree)
      pwSenders=np.concatenate(sendTree)
      srInd_1=pwReceivers
      srInd_2=pwSenders+nrParticles
      ...
      if rp["wallReceivers"]:
        wallParticleSenders=np.concatenate([srInd_1, srInd_2])
        wallParticleReceivers=np.concatenate([srInd_2, srInd_1])
      else:
      ...
      if rp["wallWeight"]==0:
        particleWallWeight=tf.ones([sampledPoints.shape[0], 1])
      elif rp["wallWeight"]==1:
        particleWallWeight=tf.tensor_scatter_nd_add(tf.zeros([sampledPoints.shape[0]]), tf.reshape(pwSenders, (-1,1)),  tf.gather(tf.tensor_scatter_nd_add(tf.zeros([nrParticles]), tf.reshape(srInd_1, (-1,1)), tf.ones([len(srInd_1)])), srInd_1))
        particleWallWeight=tf.where(particleWallWeight<0.5, 0.5, particleWallWeight)
        particleWallWeight=1.0/particleWallWeight
        particleWallWeight=tf.where(particleWallWeight>1.5, 0.0, particleWallWeight)
        particleWallWeight=tf.reshape(particleWallWeight, (-1,1))
      nrWallParticles=sampledPoints.shape[0]
      ...
      wallDistVec=-(tf.gather(sampledPoints, pwSenders)-tf.gather(currentPositionVecPerturbated, pwReceivers))
      ...
      projectedWallDistVecPerturbated=tf.reshape(tf.reduce_sum(tf.gather(currentVelocityVecPerturbated, pwReceivers)*wallUnitDistVec,1),(-1,1))*wallUnitDistVec
      ...
      angle=tf.abs(tf.reduce_sum(tf.gather(currentVelocityVecPerturbated, pwReceivers)*tf.gather(nvecs, pwSenders),1)/(tf.sqrt(tf.reduce_sum((tf.gather(currentVelocityVecPerturbated, pwReceivers))**2,1))*tf.sqrt(tf.reduce_sum((tf.gather(nvecs, pwSenders))**2,1))))
