...
parser.add_argument("-saveName", help="", type=str, default="abl2")
parser.add_argument("-addInfo", help="", type=str, default="Ab2")
...
rolloutScript=os.path.join(processGitDir, "ablations", "rolloutDrumAb2.py")

exec(open(utilScript).read(), globals())

rp=collections.OrderedDict()
rp["version"]="processDrum2Ab2"
rp["problem"]="drum"
rp["experiment"]="runs2"
rp["stepSize"]=1
...
rp["options"]="ablations/featureOptions1Ab2.py"
...
if not restart:
  ...
  saveName="d"+str(contNr)+"_"+str(int(time.time()*1000.0))+"_"+datetime.date.today().strftime("%d-%m-%Y")+"_drum2Ab2"
  if args.addInfo!="":
...
assert(rp["version"]=="processDrum2Ab2")
...
for epochNr in range(startEpoch, min(maxEpochs, maxEpochsAbort)):
  ...
  for trainNr in range(startTrainNr, min(maxMB, maxMBAbort), rp["batchSize"]):
    ...
    for batchInd in range(0, min(rp["batchSize"], len(trainList)-trainNr)):
      ...
      if rp["wallWeight"]==0:
        particleWallWeight=tf.ones([nrConstraints, 1])
      elif rp["wallWeight"]==1:
        particleWallWeight=tf.tensor_scatter_nd_add(tf.zeros([nrConstraints]), tf.reshape(takeDist[:,1], (-1,1)),  tf.gather(tf.tensor_scatter_nd_add(tf.zeros([nrParticles]), tf.reshape(srInd_1, (-1,1)), tf.ones([len(srInd_1)])), srInd_1))
