...
parser.add_argument("-saveName", help="", type=str, default="abl1")
parser.add_argument("-addInfo", help="", type=str, default="Ab1")
...
rolloutScript=os.path.join(processGitDir, "ablations", "rolloutDrumAb1.py")

exec(open(utilScript).read(), globals())

rp=collections.OrderedDict()
rp["version"]="processDrum2Ab1"
rp["problem"]="drum"
rp["experiment"]="runs2"
rp["stepSize"]=1
...
if not restart:
  ...
  saveName="d"+str(contNr)+"_"+str(int(time.time()*1000.0))+"_"+datetime.date.today().strftime("%d-%m-%Y")+"_drum2Ab1"
  if args.addInfo!="":
...
assert(rp["version"]=="processDrum2Ab1")
  ...
  for trainNr in range(startTrainNr, min(maxMB, maxMBAbort), rp["batchSize"]):
    ...
    for batchInd in range(0, min(rp["batchSize"], len(trainList)-trainNr)):
      ...
      if epochNr==0:
        ...
        bstat.trackZeros("wallVelocityVec", tuple(currentVelocityVecPerturbated.shape[1:]), nrWallParticles)
        bstat.trackZeros("wallVelocityLen", tuple(currentVelocityLenPerturbated.shape[1:]), nrWallParticles)
        bstat.trackZeros("wallAccelerationVec", tuple(currentAccelerationVecPerturbated.shape[1:]), nrWallParticles)
        bstat.trackZeros("wallAccelerationLen", tuple(currentAccelerationLenPerturbated.shape[1:]), nrWallParticles)
