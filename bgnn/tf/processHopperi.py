...
parser.add_argument("-experiment", help="", type=str, default="runsi")
...
parser.add_argument("-modulo", help="", type=int, default=100)
parser.add_argument('--train', help='', type=eval, choices=[True, False], default='False')
parser.add_argument("--trainSaveInterval", help="", type=int, default=100000)
...
rolloutScript=os.path.join(processGitDir, "rolloutHopperi.py")

exec(open(utilScript).read(), globals())

rp=collections.OrderedDict()
rp["version"]="processHopperi"
rp["problem"]="hopper"
rp["experiment"]="runsi"
rp["stepSize"]=1
...
rp["velNoise"]=(np.array([1.0, 1.0, 1.0])*1e-06*0.01).tolist()
rp["accNoise"]=(np.array([1.0, 1.0, 1.0])*1e-08*0.01).tolist()
rp["correctNoise"]=1
rp["batchSize"]=1
rp["lrSchedule"]=0
...
rp["multAngle"]=0.0
rp["gravBias"]=False
rp["model"]="modelHopperi.py"
rp["confFile"]="confHopperi.py"
...
if not restart:
  ...
  saveName="d"+str(contNr)+"_"+str(int(time.time()*1000.0))+"_"+datetime.date.today().strftime("%d-%m-%Y")+"_hopperi"
  if args.addInfo!="":
...
assert(rp["version"]=="processHopperi")
...
maxMBAbort=len(xParticleDataList[0:30])*(1001-(rp["nrPastVelocities"]+1))
...
for epochNr in range(startEpoch, min(maxEpochs, maxEpochsAbort)):
  if "trainNr" in dir() and trainNr!=min(len(trainList), maxMBAbort)-1:
    ...
    trainList=list(itertools.product(list(range(len(xParticleDataList[0:30]))), list(range((rp["nrPastVelocities"]+1)*rp["stepSize"], 1001))))
  ...
  for trainNr in range(startTrainNr, min(maxMB, maxMBAbort), rp["batchSize"]):
    ...
    for batchInd in range(0, min(rp["batchSize"], len(trainList)-trainNr)):
      ...
      particleNodeDataList.append(tf.constant(0.0, dtype=tf.float32, shape=(nrParticles,1)))
      wallNodeDataList.append(tf.constant(1.0, dtype=tf.float32, shape=(nrWallParticles,1)))
