...
if rp["useNormalVec"][0]:
  ...
  particleNodeDataList.append(     tf.constant(0.0, dtype=tf.float32, shape=(nrParticles,3))     )
  wallNodeDataList.append(     tf.gather(nvec1*(rp["multNormV"]/2.0), takeDist[:,1])*particleWallWeight     )
  wallNodeDataList.append(     tf.gather(nvec2*(rp["multNormV"]/2.0), takeDist[:,1])*particleWallWeight     )
  if dbg:
    dbgInt=20
