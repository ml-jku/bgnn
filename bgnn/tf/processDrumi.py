...
rolloutScript=os.path.join(processGitDir, "rolloutDrum.py")

exec(open(utilScript).read(), globals())

rp=collections.OrderedDict()
rp["version"]="processDrumi"
rp["problem"]="drum"
rp["experiment"]="runsi"
rp["stepSize"]=1
...
rp["multAngle"]=0.0
rp["gravBias"]=False
rp["model"]="modelDrumi.py"
rp["confFile"]="confDrumi.py"
...
if not restart:
  ...
  saveName="d"+str(contNr)+"_"+str(int(time.time()*1000.0))+"_"+datetime.date.today().strftime("%d-%m-%Y")+"_drumi"
  if args.addInfo!="":
...
assert(rp["version"]=="processDrumi")
