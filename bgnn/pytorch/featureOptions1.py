...
if rp["useNormalVec"][0]:
  posvecHash=torch.sum(torch.reshape(3**torch.arange(3, device=device),(1,3)).float()*(torch.sign(nvec)+1), 1, keepdims=True)
  negvecHash=torch.sum(torch.reshape(3**torch.arange(3, device=device),(1,3)).float()*(torch.sign(-nvec)+1), 1, keepdims=True)
  nvec1=torch.where(posvecHash>negvecHash, nvec, -nvec)
  nvec2=-nvec1
  
  
  particleNodeDataList.append(     torch.full((nrParticles,3), 0.0, device=device, dtype=torch.float32)     )
  particleNodeDataList.append(     torch.full((nrParticles,3), 0.0, device=device, dtype=torch.float32)     )
  wallNodeDataList.append(     (nvec1*(rp["multNormV"]/2.0))[takeDist[:,1]]*particleWallWeight     )
  wallNodeDataList.append(     (nvec2*(rp["multNormV"]/2.0))[takeDist[:,1]]*particleWallWeight     )
  if dbg:
    dbgInt=20
