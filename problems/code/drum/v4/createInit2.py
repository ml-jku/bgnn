...
templateGitDir=os.path.join(simGitDir, "templates", "drum", "v34")
...
openInitPar=[0]
openMainPar=[0]
scaleRadiusLower=[0.5]
scaleRadiusUpper=[0.5]
scaleLengthLower=[0.5]
scaleLengthUpper=[0.7]
nrParticles1Lower=[3000]
nrParticles1Upper=[4000]
nrParticles2Lower=[0]
nrParticles2Upper=[1]
angleInitLower=[0]
angleInitUpper=[360]
...
for simInd in range(nrSimRunsStart, nrSimRunsStop):
  ...
  tempDictInit={
    ...
    "nrParticles2": nrParticles1+nrParticles2,
  ...
  leftCap=stl.mesh.Mesh.from_file(os.path.join(inSTLPath, 'cap.stl'))
  leftCap.vectors[:,:,0]=leftCap.vectors[:,:,0]*scaleRadius
  leftCap.vectors[:,:,1]=leftCap.vectors[:,:,1]*scaleLength
  leftCap.vectors[:,:,2]=leftCap.vectors[:,:,2]*scaleRadius
  leftCap.save(os.path.join(outSTLPath, 'leftCap.stl'), mode=stl.Mode.ASCII)

  rightCap=stl.mesh.Mesh.from_file(os.path.join(inSTLPath, 'cap.stl'))
  rightCap.vectors[:,:,0]=rightCap.vectors[:,:,0]*scaleRadius
  rightCap.vectors[:,:,1]=(rightCap.vectors[:,:,1]+1.0)*scaleLength
  rightCap.vectors[:,:,2]=rightCap.vectors[:,:,2]*scaleRadius
  rightCap.save(os.path.join(outSTLPath, 'rightCap.stl'), mode=stl.Mode.ASCII)

  drum=stl.mesh.Mesh.from_file(os.path.join(inSTLPath, 'drum.stl'))
  drum.vectors[:,:,0]=drum.vectors[:,:,0]*scaleRadius
  drum.vectors[:,:,1]=drum.vectors[:,:,1]*scaleLength
  drum.vectors[:,:,2]=drum.vectors[:,:,2]*scaleRadius
  drum.save(os.path.join(outSTLPath, 'drum.stl'), mode=stl.Mode.ASCII)
