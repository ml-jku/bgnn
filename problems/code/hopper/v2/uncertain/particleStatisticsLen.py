...
runDir=os.path.join(simDir, "runs2")
...
for simInd in range(0, nrSimRunsStop):
  ...
  if not os.path.exists(os.path.join(destPath, "xData.npy")):
    xParticleDataList.append(0)
    xSceneDataList.append(0)
    continue
...
with pymp.Parallel(40) as parproc:
 for i in parproc.range(100, 130):
  print(i)
  ...
  for j in range(0, vel.shape[0]):
    overallVelSum.add(np.sqrt(np.sum(vel[j,:]**2)))
    overallVelSum2.add(np.sum(vel[j,:]**2))
  for j in range(0, acc.shape[0]):
    overallAccSum.add(np.sqrt(np.sum(acc[j,:]**2)))
    overallAccSum2.add(np.sum(acc[j,:]**2))
  ...
  destPath=destPathBase+"/"+str(i)
  np.save(os.path.join(destPath, "overallVelSum_scalar"), overallVelSum)
  np.save(os.path.join(destPath, "overallVelSum2_scalar"), overallVelSum2)
  np.save(os.path.join(destPath, "overallAccSum_scalar"), overallAccSum)
  np.save(os.path.join(destPath, "overallAccSum2_scalar"), overallAccSum2)
  np.save(os.path.join(destPath, "VelElems_scalar"), np.int64(vel.shape[0]))
  np.save(os.path.join(destPath, "AccElems_scalar"), np.int64(acc.shape[0]))
print()
