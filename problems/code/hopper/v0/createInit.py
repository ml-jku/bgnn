...
openInitPar=[0]
openMainPar=[0]
angleLower=[-3, -2, -1, 0, 1, 2, 3, 4, 5, 7, 10, 15, 20, 25, 30]
angleUpper=[-2, -1, 0, 1, 2, 3, 4, 5, 7, 10, 15, 20, 25, 30, 40]
holeSizeLower=[0.012]
holeSizeUpper=[0.038]
...
crInitPar=[[[0.85, 0.90, 0.90], [0.90, 0.95, 0.95], [0.90, 0.95, 0.95]]]
crMainPar=[[[0.85, 0.90, 0.90], [0.90, 0.95, 0.95], [0.90, 0.95, 0.95]]]
cfInitPar=[[[0.20, 0.30, 0.30],[0.30, 0.45, 0.45],[0.30, 0.45, 0.45]]]
cfMainPar=[[[0.20, 0.30, 0.30],[0.30, 0.45, 0.45],[0.30, 0.45, 0.45]]]
crfInitPar=[[[0.010, 0.015, 0.015],[0.015, 0.020, 0.020],[0.015, 0.020, 0.020]]]
crfMainPar=[[[0.010, 0.015, 0.015],[0.015, 0.020, 0.020],[0.015, 0.020, 0.020]]]
densityInitPar=[[None, [2500, 2500], [2500, 2500]]]
densityMainPar=[[None, [2500, 2500], [2500, 2500]]]
...
for simInd in range(nrSimRunsStart, nrSimRunsStop):
  ...
  tempDictMain={
    ...
    "rad": radMain,
    "tp": (np.ones((1,8))*0.12).tolist(), #np.identity(8).tolist(), #(np.ones((8,8))*0.12).tolist(),
    "templateSeed": primepool[10:18].tolist(),
    "pdistSeed": primepool[18:19].tolist(),
    "insSeed": primepool[19:20].tolist()
  }
  ...
  [f"variable density{i+1}{j+1} equal {{density[{i}][{j}]}}" for i in range(len(tempDictMain["density"]))  if tempDictMain["density"][i] is not None for j in range(len(tempDictMain["density"][i]))]+\
  [f"variable rad{i+1}{j+1}{k+1} equal {{rad[{i}][{j}][{k}]}}" for i in range(len(tempDictMain["rad"]))  if tempDictMain["rad"][i] is not None for j in range(len(tempDictMain["rad"][i])) for k in range(len(tempDictMain["rad"][i][j]))]+\
  [f"variable tp{i+1}{j+1} equal {{tp[{i}][{j}]}}" for i in range(len(tempDictMain["tp"])) for j in range(len(tempDictMain["tp"][i]))]+\
  [f"variable templateSeed{i+1} equal {{templateSeed[{i}]}}" for i in range(len(tempDictMain["templateSeed"]))]+\
  [f"variable pdistSeed{i+1} equal {{pdistSeed[{i}]}}" for i in range(len(tempDictMain["pdistSeed"]))]+\
  [f"variable insSeed{i+1} equal {{insSeed[{i}]}}" for i in range(len(tempDictMain["insSeed"]))]
