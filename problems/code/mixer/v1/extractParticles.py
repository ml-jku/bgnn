...
with pymp.Parallel(40) as parproc:
 for simInd in parproc.range(nrSimRunsStart, nrSimRunsStop):
  ...
  for j in reversed(range(0, len(strTemplsParticles))):
    ...
    x_Data=x_Data[sortOrder,:]
    id_Data=id_Data[sortOrder]
    type_Data=type_Data[sortOrder]
    radius_Data=radius_Data[sortOrder]
    density_Data=density_Data[sortOrder]
    
    if j==len(strTemplsParticles)-1:
      id_DataOld=id_Data
    else:
      mask=np.in1d(id_Data,id_DataOld)
      x_Data=x_Data[mask,:]
    ...
    x_DataArr.insert(0, x_Data)
    id_DataArr.insert(0, id_Data)
    type_DataArr.insert(0, type_Data)
    radius_DataArr.insert(0, radius_Data)
    density_DataArr.insert(0, density_Data)
